...
import org.openqa.selenium.support.ui.ExpectedConditions;
	...
	public static void testFailureLogin() throws InterruptedException {
		...
		Thread.sleep(2000);
			...
			loginfailure("instagram-login.png");
			...
			// evaluate a failure login test
				...
				public static void loginfailure(String filename) {
					
					TakesScreenshot scr= (TakesScreenshot)launch;
					
					// 2. call take screen shot method with file type
					File ss = scr.getScreenshotAs(OutputType.FILE);
					
					// 3. create  file  with screen shot
					try {
						FileHandler.copy(ss, new File("outputscreenshots\\"+filename));
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
